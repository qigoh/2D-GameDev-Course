...
@export var power := 1.0:
	set(value):
		...
		if size_curve != null:
			var sample = size_curve.sample(power)
			length = sample
...
@onready var thruster_material2 = load("res://assets/ship/main_thruster/thruster_material2.tres")
@onready var default_material = load("res://assets/ship/main_thruster/thruster_material.tres")
@onready var sideThruster = get_node("../SideThruster")
@onready var sideThruster2 = get_node("../SideThruster2")
var canBoost := true

func _ready() -> void:
	_update_drawing()
	gpu_particles_2d.emitting = Engine.is_editor_hint()
	var boostTimer = get_node("../BoostTime")
	var cdTimer = get_node("../BoostCooldown")
	boostTimer.timeout.connect(_onBoostEnd)
	cdTimer.timeout.connect(_onCooldownEnd)
...
func _process(delta: float) -> void:
	...
	if not Engine.is_editor_hint():
		...
		curl = _angle_difference_smoothed * 20.0
	if (Input.is_action_just_pressed("boost") and canBoost):
		material = thruster_material2
		sideThruster.material = thruster_material2
		sideThruster2.material = thruster_material2
		canBoost = false
...
func _update_drawing():
	...
	points = new_points
func _onBoostEnd() -> void:
	material = default_material
	sideThruster.material = default_material
	sideThruster2.material = default_material
	
func _onCooldownEnd() -> void:
	canBoost = true
